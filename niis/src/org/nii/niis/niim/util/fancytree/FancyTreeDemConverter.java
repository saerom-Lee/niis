...
public class FancyTreeDemConverter extends AbstractFancyTreeConverter {

	private List<FancyNodeData> yearNodeDatas = new ArrayList<FancyNodeData>();
	private List<FancyNodeData> securityDatas;
	private List<FancyNodeData> gridIntDatas;
	...
	public List<FancyNodeData> getFancyNodeData(List<Map<String, Object>> list, String useAt) {
		...
		for (Map<String, Object> map : list) {
			...
			if(map.get("fileExt").equals("")){
				...
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:airDetailInfo('dem')\" class=\"btnLayerOpen\"><img src=\"/niis/images/sub/ic_3.png\" alt=\"\" /></a>"));
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</span>"));
			}else{
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<span class=\"btn\">"));
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:requestImage('dem', '"+String.valueOf(map.get("xmin"))+"','"+String.valueOf(map.get("ymin"))+"','"+String.valueOf(map.get("xmax"))+"','"+String.valueOf(map.get("ymax"))
						+ "','"+(String) map.get("zoneCode")+(String) map.get("fileNam")
						+ "')"
						+ "\"><img src=\"/niis/images/sub/ic_1.png\" alt=\"\" /></a> "));
				if (useAt.equals("00") || useAt.equals("02")){
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:applicationList(2,'demTreeRoot')\" class=\"btnLayerOpen\"><img src=\"/niis/images/sub/ic_2.png\" alt=\"\" /></a> "));					
				}
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:openDemMetadataLayer('"+(String) map.get("zoneNam")+"','"+(String) map.get("map5000Num")+"','"+(String) map.get("zoneCode")+"')\" class=\"btnLayerOpen\"><img src=\"/niis/images/sub/ic_3.png\" alt=\"\" /></a>"));
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</span>"));
			}
		...
		try{
			for (Map<String, Object> map : list) {
				...
				if(isExitsChildren("gridInt", gridIntDatas, map)){
					...
					FancyNodeData fancyNodeData = new FancyNodeData();
					fancyNodeData.setTitle(String.valueOf(map.get("gridInt")));
					fancyNodeData.setFolder(true);
					fancyNodeData.setSelected(true);
					if (index == 0){
						fancyNodeData.setExpanded(true);
				...
				if (isExitsChildren("zoneCode", zoneNodeDatas, map)) {
					...
					fancyNodeData.setTitle((String) map.get("zoneNam"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<span class=\"btn\">"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:getDemZoneMarker('" + map.get("securityCd") + "', '" + map.get("gridInt") + "', '" + map.get("zoneCode") + "', '')\" >"));
				...
				if (isExitsChildren("mapNum", mapNodeDatas, map)) {
					...
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<span class=\"btn\">"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:getDemZoneMarker('" + map.get("securityCd") + "', '" + map.get("gridInt") + "', '" + map.get("zoneCode") + "', '" + map.get("mapNum") + "')\" >"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<img src=\"" + imgPath + "/icon/ic_3.png\" alt=\"\" />"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</a>"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</span>"));
					fancyNodeData.setFolder(true);
					fancyNodeData.setExpanded(false);
					fancyNodeData.setExtraClasses("dem_" + map.get("zoneCode") + "_" + map.get("gridInt") + "_" + map.get("securityCd") + "_" + map.get("mapNum"));
	
					Map<String, Object> key = new HashMap<String, Object>();
					key.put("key", map.get("mapNum"));
					key.put("imageCde", map.get("imageCde"));
					key.put("zoneCode", map.get("zoneCode"));
					key.put("gridInt", map.get("gridInt"));
