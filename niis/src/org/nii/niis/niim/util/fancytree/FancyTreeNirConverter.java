...
public class FancyTreeNirConverter extends AbstractFancyTreeConverter {
		...
		for (Map<String, Object> map : list) {
			...
			if(map.get("fileExt").equals("")){
				...
				if (useAt.equals("00") || useAt.equals("02")){
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:applicationList(2,'nirTreeRoot')\" class=\"btnLayerOpen\"><img src=\"/niis/images/sub/ic_2.png\" alt=\"\" /></a> "));
				}
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:openMetadataLayer(this)\" class=\"btnLayerOpen\"><img src=\"/niis/images/sub/ic_3.png\" alt=\"\" /></a>"));
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</span>"));
			}
		...
		try{
			for (Map<String, Object> map : list) {
				...
				if(isExitsChildren("zoneCode", zoneNodeDatas, map)){
					...
					fancyNodeData.setTitle((String) map.get("zoneNam"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<span class=\"btn\">"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:getNirZoneMarker('" + map.get("securityCd") + "', '" + map.get("zoneCode") + "', '')\" >"));
				...
				if(isExitsChildren("phCourse", courseNodeDatas, map)){
					...
					fancyNodeData.setTitle((String) map.get("phCourse"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<span class=\"btn\">"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:getNirZoneMarker('" + map.get("securityCd") + "', '" + map.get("zoneCode") + "', '" + map.get("phCourse") + "')\" >"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<img src=\"" + imgPath + "/icon/ic_3.png\" alt=\"\" />"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</a>"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</span>"));
					fancyNodeData.setFolder(true);
					fancyNodeData.setExpanded(false);
					fancyNodeData.setExtraClasses("nir_" + map.get("zoneCode") + "_" + map.get("phCourse") + "_" + map.get("securityCd"));
	...
	public List<FancyNodeData> getFancyDataNode(List<Map<String, Object>> list, String useAt, String imgPath) throws Exception {
		try{
			...
			for (Map<String, Object> map : list) {
				...
				String xmin = String.valueOf(map.get("xmin"));
				String ymin = String.valueOf(map.get("ymin"));
				String xmax = String.valueOf(map.get("xmax"));
				String ymax = String.valueOf(map.get("ymax"));
				String path = String.valueOf(map.get("nixPath"));
				
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<span class=\"btn\">"));
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:requestImage('air', '"+xmin+"', '"+ymin+"', '"+xmax+"', '"+ymax+"', '"+path+"') \">"));
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<img src=\"" + imgPath + "/sub/ic_1.png\" alt=\"이미지 보기\" title=\"이미지 보기\" />"));
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</a>"));
				/*if(map.get("fileExt").equals("O") && imgPath.indexOf("niim") > -1 && (useAt.equals("00") || useAt.equals("02"))){
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:applicationList(2, 'nirTreeRoot')\" >"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<img src=\"" + imgPath + "/sub/ic_2.png\" alt=\"개별다운로드\" title=\"개별다운로드\" />"));
					fancyNodeData.setTitle(fancyNodeData.getTitle().concat("</a>"));
				}*/
				fancyNodeData.setTitle(fancyNodeData.getTitle().concat("<a href=\"javascript:openAirMetadataLayer('"+(String) map.get("zoneNam")+"', '"+(String) map.get("zoneCode")+"', '"+(String) map.get("phCourse")+"', '"+(String) map.get("phNum")+"')\" >"));
