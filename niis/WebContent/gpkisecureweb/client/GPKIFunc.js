...
function Init()
{
	if (GPKISecureWeb==null || typeof(GPKISecureWeb)=='undefined') {
		location.href = clientInstallPath;
		return -1;
	} else {
		if (needInstall()) {
			location.href = clientInstallPath;
...
function LoginLink2(link,sessionid)
{	
	var strData;
	var nResult;
	var strReturnData;
...
//		document.gpkiForm.action = params[1].substring(params[1].lastIndexOf('\/')+1,params[1].length-1);
		document.gpkiForm.action = "/niis/gpkisecureweb/jsp/gpkiLoginCheck.jsp";
...
function actionLoginLink2(strData){
	var nResult;
	var strReturnData;
	...
	if( nResult == 0)
	{
		document.gpkiForm.encryptedData.value = replaceLtRt(strReturnData);
//		document.gpkiForm.action = params[1].substring(params[1].lastIndexOf('\/')+1,params[1].length-1);
//		var pop_title = "registerPopup" ;
//		window.open("", pop_title) ;
//		document.gpkiForm.target = pop_title;
		document.gpkiForm.action = "/niis/gpkisecureweb/jsp/register.jsp";
//		if ( params[2] == "" || params[2] == null ) {
//			document.gpkiForm.target="_self";
//		}else{
//			document.gpkiForm.target=params[2];
...
{	
	...
	if( nResult == 0 ) 
	{
		...
		var target = '/niis/gpkisecureweb/jsp/makeDSKeySecure';
		target += '.'+addPageExt();		
...
function initSecureSession(sessionid){
	...
	if( (nResult != -1) && (setSID == 0) && (resEnc == 30053) )
		location.href = "/niis/gpkisecureweb/jsp/requestSecureSession." + serverLangExt + "?rnd=" + sessionid;
}
