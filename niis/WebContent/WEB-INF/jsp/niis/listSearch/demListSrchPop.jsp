...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		//상단 체크박스 클릭 이벤트
		$("#demCfrmHead").on("click", "input[type=checkbox]", function(){
			
			var isChecked = $(this).is(":checked");
			
			$("#demCfrmList input[type=checkbox]").each(function(i){
	...
	function regApplyPop(){
		...
		if($("input[id=cfrmDem]:checked").length < 1){
			alert("영상을 선택해 주세요.");
			return;
		}
		...
		$("input[id=cfrmDem]").each(function(){
			if($(this).is(":checked")){
...
<div class="boardStyle_2" style="max-height: 319px; overflow: auto;">
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tbody id="demCfrmList">
			<c:forEach items="${chkDemList}" var="list">
			<tr>
				<td>
					<input type="checkbox" id="cfrmDem" name="cfrmDem" checked />
